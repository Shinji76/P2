...
void ResultsWidget::showInitialResults() {
    ...
    for(auto it = boxes.begin(); index < 4; it++) {
        (*it)->show();
        grid->addWidget(*it, index / 2, index % 2);
        index++;
    }
    page_index = 1;
    previous_page->setEnabled(false);
...
void ResultsWidget::showNextPage() {
    ...
    for(auto it = boxes.begin() + page_index*4; index < 4; it++) {
        if(it != boxes.end()) {
            (*it)->show();
            grid->addWidget(*it, index / 2, index % 2);
        }
            ...
            for(; index < 4; index++) {
            QSpacerItem* emptySpace = new QSpacerItem(150, 150);
            grid->addItem(emptySpace, index / 2, index % 2);
            }
        }
        index++;
    }
